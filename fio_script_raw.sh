...
direct=0  #0: buffered , 1:directIO
	...
	#Result - ECHO
	echo ${nodename}","${bsize}","${numjobs}",1,"${iternum}","${output} > /mnt/share/cykim/result/${nodename}.txt
	...
	echo "["${nodename}",BJ="${bsize}",NJ="${numjobs}",IT-"${iternum}"],99%=["${per99}"],iops_avg=["${iopsavg}"],TH=["${th}"]"
	sleep 1
