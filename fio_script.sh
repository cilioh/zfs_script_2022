...
direct=0  #0: buffered , 1:directIO
	...
	#echo 'throughput-size-clat_min-clat_max-clat_avg-lat_min-lat_max-lat_avg'

	output=`fio --directory=${directory} --name=${filename}${iternum} --rw=write --direct=${direct} --bs=${blsize} --size=${bsize} --numjobs=${numjobs} --group_reporting --fallocate=none`
	#echo $output
	output=`echo $output | awk '$1 == "WRITE:" { print $3 } $1 == "iops" { print } /9[5-9].[0-9]+th/ { print }' | grep -oP '(\([0-9.]+|95.00th=\[[\s\d]+|99.00th=\[[\s\d]+|99.90th=\[[\s\d]+|99.99th=\[[\s\d]+|min=[\s\d.]+|max=[\s\d.]+|avg=[\s\d.]+)' | grep -oP '([\(\[]\s*(\d+^[.]|\d+.\d+)|=\s*(\d+^[.]|\d+.\d+))' | grep -oP '(\d+^[.]|\d+.\d+)' | tr '\n' ' ' | awk '{ print $1"," $2"," $3"," $4"," $5"," $6"," $7"," $8}'`
	echo ${nodename}","${bsize}","${numjobs}","${stripecount}","${iternum}","${output} > /mnt/share/cykim/result/${nodename}.txt

	#echo $output
	th=`echo $output | cut -d',' -f1`
	#echo $th
	lat_avg=`echo $output | cut -d',' -f8`
	#echo $lat_avg
	echo "["${nodename}",BJ="${bsize}",BS="${blsize}",Xfer="${xfersize}",NJ="${numjobs}",SC="${stripecount}",IT-"${iternum}"],lat_avg=["${lat_avg}"],TH=["${th}"]"
#
:<<'END'
	echo '======='


	output=`fio --directory=${directory} --name=${filename}${iternum} --rw=write --direct=${direct} --bs=${blsize} --size=${bsize} --numjobs=${numjobs} --group_reporting --fallocate=none | awk '$1 == "WRITE:" { print $3 } $1 == "iops" { print } /9[5-9].[0-9]+th/ { print }' | grep -oP '(\([0-9.]+|95.00th=\[[\s\d]+|99.00th=\[[\s\d]+|99.90th=\[[\s\d]+|99.99th=\[[\s\d]+|min=[\s\d.]+|max=[\s\d.]+|avg=[\s\d.]+)' | grep -oP '([\(\[]\s*(\d+^[.]|\d+.\d+)|=\s*(\d+^[.]|\d+.\d+))' | grep -oP '(\d+^[.]|\d+.\d+)' | tr '\n' ' ' | awk '{ print $1"," $2"," $3"," $4"," $5"," $6"," $7"," $8}'`
