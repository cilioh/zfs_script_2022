...
do
	...
	output=`nvme intel smart-log-add /dev/nvme0n1 | grep '^[nh][ao][ns][dt]' | awk '{ print $5 }'`
	user_writes_bf_0=`expr $(echo $output | awk '{ print $2 }') + 0`
	nand_writes_bf_0=`expr $(echo $output | awk '{ print $1 }') + 0`

	output=`nvme intel smart-log-add /dev/nvme1n1 | grep '^[nh][ao][ns][dt]' | awk '{ print $5 }'`
	user_writes_bf_1=`expr $(echo $output | awk '{ print $2 }') + 0`
	nand_writes_bf_1=`expr $(echo $output | awk '{ print $1 }') + 0`

	output=`nvme intel smart-log-add /dev/nvme2n1 | grep '^[nh][ao][ns][dt]' | awk '{ print $5 }'`
	user_writes_bf_2=`expr $(echo $output | awk '{ print $2 }') + 0`
	nand_writes_bf_2=`expr $(echo $output | awk '{ print $1 }') + 0`

	output=`nvme intel smart-log-add /dev/nvme3n1 | grep '^[nh][ao][ns][dt]' | awk '{ print $5 }'`
