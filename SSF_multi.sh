...
#				do
					...
					lfs setstripe -C ${numjobs} /mnt/lustre
					#lfs setstripe -c 1 /mnt/lustre

					for nodes in "CN8" "CN9" "CN10"  #"CN11" "CN12"
					do
						ssh $nodes "lfs setstripe -C "${numjobs}" /mnt/lustre"
						#ssh $nodes "lfs setstripe -c 1 /mnt/lustre"
					done

					for iter in {1..1}  #iteration
					do
						rm -rf /mnt/lustre/*
						sleep 5
						...
						/usr/local/bin/mpirun -np ${numjobs} ior -w -t=${xfersize} -b=${bsize} -o /mnt/lustre/pine &
						ssh cn8 "/usr/local/bin/mpirun -np ${numjobs} ior -w -t=${xfersize} -b=${bsize} -o /mnt/lustre/banana" &
						ssh cn9 "/usr/local/bin/mpirun -np ${numjobs} ior -w -t=${xfersize} -b=${bsize} -o /mnt/lustre/citrus" &
						ssh cn10 "/usr/local/bin/mpirun -np ${numjobs} ior -w -t=${xfersize} -b=${bsize} -o /mnt/lustre/dragon"
